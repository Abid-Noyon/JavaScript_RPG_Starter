...
function buyWeapon() {
    if (currentWeapon < weapons.length - 1) {
        if (gold >= 30) {
            ...
            invertory.push(newWeapon);
            text.innerText += " In your invertory you have " + invertory;
        } else {
...
function sellWeapon() {
    if (invertory.length > 1) {
        gold += 15;
        goldText.innerText = gold;
        let currentWeapon = invertory.shift();
...
function attack() {
    ...
    if (Math.random() <= .1 && invertory.length !== 1) {
        text.innerText += " Your " + invertory.pop() + " breaks.";
        currentWeapon--;
    }
}
